...
function getDayName(date) {
  ...
  return data.toLocaleString('en', { weekday: 'long' });
}
...
function getNextFriday(date) {
  ...
  if (day === 5) {
    ...
    data.setDate(data.getDate() + ((5 - day + 7) % 7));
  }
  return data;
}
...
function formatDate(date) {
  ...
  const h = addZero(data.getUTCHours());
  const m = addZero(data.getUTCMinutes());
  const s = addZero(data.getUTCSeconds());
  const a = h >= 12 ? 'PM' : 'AM';
  return `${M}/${D}/${Y}, ${h % 12 || 12}:${m}:${s} ${a}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber =
    Math.round((thursday - firstThursday) / (7 * 86400000)) + 1;
